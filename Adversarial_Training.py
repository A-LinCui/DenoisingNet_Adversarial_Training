...
parser.add_argument('-GPU', dest='GPU', type=str, default="0", help='used GPU')
...
with open(args.model_address + '_adv_training_log.csv', 'w') as f:
    writer = csv.writer(f)
...
scheduler = MultiStepLR(optimizer, milestones=[int(args.epoches/2), int(args.epoches*3/4), int(args.epoches*7/8)], gamma=0.1)
...
for epoch in range(args.epoches):
    ...
    # Setup the adversary
    fmodel = foolbox.models.PyTorchModel(model, bounds=(-255, 255), num_classes=10)
    ...
    if test_adv_acc > best_test_adv_acc:
        ...
        torch.save(model.state_dict(), args.model_address + '_adv.pkl')
    ...
    print("epoch:{} clean_test_acc: {:.3f}  adv_test_acc: {:.3f}".format(epoch, test_acc, test_adv_acc))
    ...
    print("         train_clean_loss: {:.3f} train_adv_loss: {:.3f} train_total_loss: {:.3f}".format(train_clean_loss, train_adv_loss, train_loss))
    print("         clean_train_acc: {:.3f}  adv_train_acc: {:.3f}".format(train_acc, train_adv_acc))
    print("         Consumed time on the testset: {:.5f}s  on the trainset: {:.5f}s\n".format(testset_total_time, trainset_total_time))

    # Recond the statistics
    with open(args.model_address + '_adv_training_log.csv', 'a') as f:
        writer = csv.writer(f)
    ...
    epochs, test_acc, test_adv_acc, train_clean_loss, train_adv_loss, train_loss, train_acc, train_adv_acc = [], [], [], [], [], [], [], []
    with open(args.model_address + '_adv_training_log.csv', 'r') as f:
        reader = csv.reader(f)
        flag = 0
        for i in reader:
            if flag == 0:
                ...
                test_acc.append(float(i[1]))
                test_adv_acc.append(float(i[2]))
                train_clean_loss.append(float(i[3]))
                train_adv_loss.append(float(i[4]))
                train_loss.append(float(i[5]))
                train_acc.append(float(i[6]))
                train_adv_acc.append(float(i[7]))

    # Generate the training accuracy line
    plt.figure(num=0)
    plt.plot(epochs, test_acc, label='test_accuracy', linewidth=2, color='r')
    plt.plot(epochs, test_adv_acc, label='test_adv_accuracy', linewidth=2, color='b')
    plt.plot(epochs, train_acc, label='train_accuracy', linewidth=2, color='y')
    plt.plot(epochs, train_adv_acc, label='train_adv_accuracy', linewidth=2, color='g')
    plt.xlabel("epoch")
    plt.ylabel("accuracy")
    ...
    plt.figure(num=1)
    plt.plot(epochs, train_loss, label='train_total_loss', linewidth=2, color='r')
    plt.plot(epochs, train_clean_loss, label='train_clean_loss', linewidth=2, color='b')
    plt.plot(epochs, train_adv_loss, label='train_adv_loss', linewidth=2, color='y')
    plt.xlabel("epoch")
    plt.ylabel("loss")
    plt.xlim(-1, max(epochs) + 1)
